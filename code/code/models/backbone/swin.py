...
def CSWin_tiny(pretrain=None):
    model = CSWin(patch_size=4, embed_dim=64, depth=[1,2,21,1], img_size=256,
        split_size=[1,2,7,7], num_heads=[2,4,8,16], mlp_ratio=4.)
    if pretrain is not None:
        old_dict = torch.load(pretrain)['state_dict']
...
def CSwin_small(pretrain=None):
    model = CSWin(patch_size=4, embed_dim=64, depth=[2,4,32,2], img_size=256,
        split_size=[1,2,7,7], num_heads=[2,4,8,16], mlp_ratio=4.)
    if pretrain is not None:
        old_dict = torch.load(pretrain)['state_dict']
        model_dict = model.state_dict()
...
def CSwin_base(pretrain=None):
    model = CSWin(patch_size=4, embed_dim=96, depth=[2,4,32,2], img_size=256,
...
if __name__ == '__main__':
    model = CSWin_tiny(pretrain='/home/ai1007/airs/semi/code/models/pretrain/backbone/upernet_swin_tiny_patch4_window7_512x512.pth')
